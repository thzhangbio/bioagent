...
- 本次导入的 `literature` 集合，会替换掉知识库里原先的 `literature` 集合
- 如果你只导入一部分文献，旧文献会从 `literature` 集合中消失
...
flowchart TD
    ...
    C --> D["knowledge-import --source literature_kb --collection literature --mode replace-collection"]
...
当前 `references` 不会被强过滤，只是被标记成 `sectionType: references` 后继续入库。
...
- [`03-ingest-index.ts`](/Users/tianhui/Webstart/bioagent/side-tools/pdf-text-cleanup/pyramid/segment-out-to-knowledge/03-ingest-index/03-ingest-index.ts)
- 把 `replace-collection` 改成 `upsert-by-source-id`
