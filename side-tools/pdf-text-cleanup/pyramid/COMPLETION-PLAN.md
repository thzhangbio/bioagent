...
- [x] 为 `segment-inbox-to-out/` 建立根总控代码文件
- [x] 设计并落地段Ⅰ统一上下文对象
- [x] 让段Ⅰ总控按固定顺序调度 `00` 到 `12`
- [x] 确定段Ⅰ总控的最小可运行入口
...
- [x] 让 `pyramid/` 内实现不再依赖侧车根目录旧代码文件
- [x] 将段级总控文件改为与根目录同名
- [x] 将各子文件夹根总控文件改为与子文件夹同名
- [x] 将命令脚本直接改为指向 `pyramid/` 新路径
- [x] 将已迁移的旧根文件从 `pdf-text-cleanup/` 目录移除
